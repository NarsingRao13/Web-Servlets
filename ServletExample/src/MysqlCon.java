...
public class MysqlCon {
	...
	public void callDataBase(String n, String p) {
		try {
			...
			Connection conob = DriverManager.getConnection(
					"jdbc:mysql://localhost:3306/narsing?characterEncoding=latin1", "root", "n@rsingK#13");

			PreparedStatement ps = conob.prepareStatement("insert into registration values(?,?)");
			ps.setString(1, n);
			ps.setString(2, p);
	...
	public void login() throws SQLException {
		...
		try {
			...
			Connection conob = DriverManager.getConnection(
					"jdbc:mysql://localhost:3306/narsing?characterEncoding=latin1", "root", "n@rsingK#13");
			Statement stm = conob.createStatement();
			// List<UserDetails> loginCredentials = new
